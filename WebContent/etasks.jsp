...
<%
	...
	String sql="select id,title,status,uid,priority,createdon from tasks WHERE pid=? and uid=?";
	if(!uid.equals("0"))
		sql+=" and uid='"+uid+"'";
	if(!pr.equals("0"))
		sql+=" and priority='"+pr+"'";
	if(!st.equals("0"))
		sql+=" and status='"+st+"'";
	List<Map<String,String>> data=DbHelper.executeDQL(sql, pid,uid);
%>
        ...
        <div class="card-body">
        	...
        	<form class="float-right form-inline mr-2">
				...
				<select name="assignee" class="form-control form-control-sm mr-2">
					...
					%>
					<option <%= uid.equals(pp.get("id"))?"selected":"" %> value="<%= pp.get("id") %>">
					<%= pp.get("uname").equals(user)?"Current User" : pp.get("uname") %>
					</option>
					<% } %>
				</select>
			...
			<div class="modal-body">			
				...
				<div class="form-group form-row">
					...
					<select name="uid" required class="form-control form-control-sm">
						...
						<%
						List<Map<String,String>> mgrs=DbHelper.executeDQL("SELECT id,uname,designation FROM users WHERE role!=?", "A");
